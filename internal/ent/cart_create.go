...
func (cc *CartCreate) SetUsername(s string) *CartCreate {
	cc.mutation.SetUsername(s)
	return cc
}

// SetCart sets the "cart" field.
func (cc *CartCreate) SetCart(si []schema.CartItem) *CartCreate {
	cc.mutation.SetCart(si)
	return cc
}
...
func (cc *CartCreate) check() error {
	if _, ok := cc.mutation.Username(); !ok {
		return &ValidationError{Name: "username", err: errors.New(`ent: missing required field "Cart.username"`)}
	}
	if _, ok := cc.mutation.Cart(); !ok {
		return &ValidationError{Name: "cart", err: errors.New(`ent: missing required field "Cart.cart"`)}
	}
	return nil
}
...
func (cc *CartCreate) createSpec() (*Cart, *sqlgraph.CreateSpec) {
	...
	if value, ok := cc.mutation.Cart(); ok {
